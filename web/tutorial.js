...
export function initTutorial() {
  ...
  if (listEl) {
    listEl.innerHTML = TUTORIAL_DATA.map((s, i) =>
      `<div class="tut-nav-item" data-i="${i}"><svg class="tut-nav-icon" aria-hidden="true"><use href="./sprites.svg#${tutIcon(s.title)}"/></svg><span class="tut-nav-text">${s.title}</span></div>`).join('');
    listEl.onclick = e => {
